...
describe("PDF URL conversion", () => {
  ...
  test("converts arXiv abstract URL with version to PDF URL", () => {
    const abstractUrl = "https://arxiv.org/abs/2505.01441v2";
    const expected = "https://arxiv.org/pdf/2505.01441v2";
    const result = getPdfUrl(abstractUrl);
    expect(result).toBe(expected);
  });
  ...
  test("converts bioRxiv abstract URL with extra query params to PDF URL", () => {
    const abstractUrl =
      "https://www.biorxiv.org/content/10.1101/2025.07.24.666528v1?rss=1&utm_source=test&utm_medium=rss";
    const expected = "https://www.biorxiv.org/content/10.1101/2025.07.24.666528v1.full.pdf";
    const result = getPdfUrl(abstractUrl);
    expect(result).toBe(expected);
  });
  ...
  test("throws error for arxiv.com domain (not arxiv.org)", () => {
    const unsupportedUrl = "https://arxiv.com/abs/2505.01441";
    expect(() => getPdfUrl(unsupportedUrl)).toThrow("Unsupported URL format");
  });
});
